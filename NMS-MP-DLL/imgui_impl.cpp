...
//#ifdef _WIN32
	io.ImeWindowHandle = WindowFromDC(window_HDC);
...
void ImGui_NewFrame()
{
	...
	int w, h;
	HWND hwnd = WindowFromDC(window_HDC);
	RECT windowRect;
	...
	// Setup time step
	long double current_time = std::chrono::system_clock::now().time_since_epoch().count();
